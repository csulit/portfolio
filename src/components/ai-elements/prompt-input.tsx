...
interface FileValidationResult {
  valid: File[];
  error?: { code: "accept" | "max_file_size" | "max_files"; message: string };
  capacityExceeded?: boolean;
}

function validateAndCapFiles(
...
export const PromptInput = ({
  ...
  const addLocal = useCallback(
    (fileList: File[] | FileList) => {
      setItems((prev) => {
        const result = validateAndCapFiles(
          fileList,
          matchesAccept,
          maxFileSize,
          maxFiles,
  ...
  const addWithProviderValidation = useCallback(
    (fileList: File[] | FileList) => {
      ...
      if (result.valid.length > 0) {
        controller?.attachments.add(result.valid);
      }
    },
